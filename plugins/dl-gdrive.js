...
const handler = async (m, {conn, args, usedPrefix, command}) => {
  if (!args[0]) throw `_*< DOWNLOADER - GDRIVE />*_\n\n*[ ℹ️ ] Masukkan tautan Google Drive.*`;
  try {
    GDriveDl(args[0]).then(async (res) => {
      conn.reply(m.chat, '_*< DOWNLOADER - GDRIVE />*_\n\n*[ ℹ️ ] File sedang dikirim. Tunggu...*\n\n*[ ℹ️ ] Jika tidak dikirimkan, bisa jadi karena melebihi batas ukuran.*', m);
      if (!res) throw res;
      conn.sendFile(m.chat, res.downloadUrl, res.fileName, '', m, null, {mimetype: res.mimetype, asDocument: true});
    });
  } catch (e) {
    m.reply('_*< DOWNLOADER - GDRIVE />*_\n\n*[ ℹ️ ] Terjadi kesalahan. Silakan coba lagi nanti.*');
    console.log(e);
  }
};
