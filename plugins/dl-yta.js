...
const handler = async (m, {text, conn, args, usedPrefix, command}) => {
      ...
      if (Array.isArray(global.videoList) && global.videoList.length > 0) {
        ...
        if (matchingItem) {
          if (index < matchingItem.urls.length) {
            ...
            throw `*[❗] Tautan untuk nomor tersebut tidak ditemukan, masukkan angka antara 1 dan 1. ${matchingItem.urls.length}*`;
          }
        } else {
          throw `*[❗] Gunakan perintah dengan cara (${usedPrefix + command} <angka>), Silakan cari video dengan perintah ${usedPrefix}playlist <teks>*`;
        }
      } else {
        throw `*[❗] Gunakan perintah dengan cara (${usedPrefix + command} <angka>), Silakan cari video dengan perintah ${usedPrefix}playlist <teks>*`;
      }
  ...
  const { key } = await conn.sendMessage(m.chat, {text: `*_⏳ Audio Anda sedang diproses...⏳_*`}, {quoted: m});
  try {
   ...
   if (fileSizeInMB > 50) {
    ...
    await conn.sendMessage(m.chat, {text: `*[ ✔ ] Audio berhasil diunduh dan dikirim.*\n\n*—◉ Dikirim dalam format dokumen karena berat audionya ${roundedFileSizeInMB} MB dan melebihi batas yang ditetapkan oleh WhatsApp.*\n*◉ Title:* ${ttl_1}`, edit: key}, {quoted: m});
    enviando = false
   } else {
    await conn.sendMessage(m.chat, {audio: buff, caption: `*▢ Title:* ${ttl_1}\n*▢ Size:* ${roundedFileSizeInMB} MB`, fileName: ttl_1 + '.mp3', mimetype: 'audio/mpeg'}, {quoted: m});
    await conn.sendMessage(m.chat, {text: `*[ ✔ ] Audio berhasil diunduh dan dikirim.*`, edit: key}, {quoted: m});
    enviando = false   
   }    
  } catch {
    console.log('noooooo')
  try {
    ...
    await conn.sendMessage(m.chat, {text: '*[ ✔ ] Audio berhasil diunduh.*', edit: key}, {quoted: m});
  } catch {
    try {
      ...
      try {
        ...
        await conn.sendMessage(m.chat, {text: `*[ ❌ ] Audio tidak dapat diunduh atau dikirim, coba lagi.*`, edit: key}, {quoted: m});
        throw '*[❗] Kesalahan, audio tidak dapat diunduh.*';
      }
    }
  }
}};
