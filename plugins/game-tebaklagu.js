...
RESPONDE A ESTE MENSAJE CON LAS RESPUESTAS!`.trim();
  ...
  const aa = await conn.sendMessage(m.chat, {audio: {url: json.lagu}, fileName: `error.mp3`, mimetype: 'audio/mpeg'}, {quoted: m});
  if (!aa) return conn.sendFile(m.chat, json.lagu, 'coba-lagi.mp3', '', m);
};
