...
const handler = async (m, {conn, args, usedPrefix, command}) => {
  const mat =`_*< GAMES - MATHEMATICS />*_\n\n*[ ℹ️ ] Anda perlu menentukan tingkat kesulitan yang ingin Anda mainkan.*\n*Contoh:* _${usedPrefix + command} medium_\n\n*Tingkat yang tersedia:* ${Object.keys(modes).join(' | ')}`.trim();
...
['𝙼𝙰𝚃𝙴𝚂 𝙷𝙰𝚁𝙳', `${usedPrefix + command} hard`]], m)*/
  ...
  const id = m.chat;
  if (id in global.math) return conn.reply(m.chat, '_*< GAMES - MATHEMATICS />*_\n\n*[ ℹ️ ] Masih ada soal matematika yang belum terselesaikan dalam obrolan ini.*', global.math[id][0]);
  const math = genMath(mode);
  global.math[id] = [
    await conn.reply(m.chat, `_*< GAMES - MATHEMATICS />*_\n\n*Question:* ${math.str}\n*Time:* ${(math.time / 1000).toFixed(2)}s\n*Award:* ${math.bonus} XP\n\n`, m),
    math, 4,
    setTimeout(() => {
      if (global.math[id]) {
