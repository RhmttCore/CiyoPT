...
handler.before = async (m) => {
  ...
  if (bot.modoia && !m.isGroup && !Prefijo && !m.fromMe && m.text !== '') {
     if (/^.*false|disnable|(turn)?off|0/i.test(m.text)) return;
        ...
        const namedem = `${name || 'Undefined'}`
        const sytm = await fetch(`https://raw.githubusercontent.com/RhmttCore/CiyoBotV2/master/src/JSON/chatgpt_indicciones.txt`).then(v => v.text());
        const sistema1 = sytm.replace('@name', namedem)
        const sistema2 = 'Anda akan menjadi CiyoBot, bot WhatsApp yang dibuat oleh RhmttDev';
        try {
