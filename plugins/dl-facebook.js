...
const handler = async (m, {conn, args, command, usedPrefix}) => {
  if (!args[0]) throw `_*< DOWNLOADER - FACEBOOK />*_\n\n*[ ℹ️ ] Masukkan tautan video facebook.*\n\n*[ 💡 ] Contoh:* _${usedPrefix + command} https://fb.watch/12345678/_`;
  if (!args[0].match(/www.facebook.com|m.facebook|fb.watch/g)) throw `_*Masukkan tautan dengan benar*_`;
  try {
    await m.reply(`_*< DOWNLOADER - FACEBOOK />*_\n\n*[ ℹ️ ] Sedang diproses...*`);
    ...
    try {
      ...
      try {
        ...
        conn.sendFile(m.chat, VIDEO, 'error.mp4', ``, m);
      } catch (err2) {
        try {
          const ress = await fg.fbdl(args[0]);
          const urll = await ress.data[0].url;
          await conn.sendFile(m.chat, urll, 'error.mp4', '', m);
        } catch (err3) {
          try {
            const res = await fbDownloader(args[0]);
            for (const result of res.download) {
              const ur = result.url;
              await conn.sendFile(m.chat, ur, 'error.mp4', '', m);
            }
          } catch (err4) {
            try {
