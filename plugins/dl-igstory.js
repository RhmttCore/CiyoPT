const handler = async (m, {conn, args, usedPrefix, command}) => {
  if (!args[0]) throw `*[❗] Username Instagram target diperlukan*`;
  await m.reply(global.wait);
  const res = await fetch(`https://api.lolhuman.xyz/api/igstory/${args[0]}?apikey=${lolkeysapi}`);
  const anu = await res.json();
  const anuku = anu.result;
  if (anuku == '') return m.reply('*[❗] Target tidak mempunyai story saat ini.*');
  for (const i of anuku) {
    const res = await axios.head(i);
    const mime = res.headers['content-type'];
    if (/image/.test(mime)) {
      await conn.sendFile(m.chat, i, 'error.jpg', null, m).catch(() => {
        return m.reply('*[❗] Tidak dapat menemukan story target*');
      });
    }
    if (/video/.test(mime)) {
