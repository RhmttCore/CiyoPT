...
const handler = async (m, { conn, usedPrefix, command, text, args }) => {
  ...
  if (!efecto) {
    ...
    let responseText = `*[❗] Anda belum memasukkan efek, silakan masukkan efek suara.*\n\n*—◉ Pilih salah satu efek berikut:*\n`;

    for (let i = 0, count = 0; count < 100 && i < voiceList.resultado.length; i++) {
      const entry = voiceList.resultado[i];
      if (entry.ID.length <= 20) {
        responseText += `*◉ ${usedPrefix + command} ${entry.ID} teks_anda*\n`;
        count++;
      }
    }
  ...
  if (!efectoValido) return conn.sendMessage(m.chat, { text: `*[❗] Efek yang diberikan tidak ada dalam daftar, gunakan ${usedPrefix + command} untuk daftar efek.*` }, { quoted: m });

  if (!texto) return conn.sendMessage(m.chat, {text: `*[❗] Masukkan teks yang ingin Anda ubah menjadi audio.*\n\n*—◉ Contoh:*\n*◉ ${usedPrefix + command} ${efecto} Halo, ini adalah contoh penggunaan perintah.*`}, {quoted: m});
...
async function getVoiceList() {
  ...
  try {
    ...
    return { resultado: simplifiedList ? simplifiedList : '[❗] Kesalahan, tidak ada respons dari server.' };
  } catch (error) {
    console.error('Error:', error);
    return { resultado: '[❗] Kesalahan, tidak ada respons dari server.' };
    throw error;
  }
}
...
async function makeTTSRequest(texto, efecto) {
  ...
  try {
    ...
    return { resultado: url ? url : '[❗] URL tidak ditemukan sebagai tanggapan.' };
  } catch (error) {
    console.error('Error:', error);
    return { resultado: '[❗] Kesalahan, tidak ada respons dari server.' };
  }
