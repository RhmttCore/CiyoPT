...
const handler = async (m, {conn, args, usedPrefix, command}) => {
      ...
      if (Array.isArray(global.videoList) && global.videoList.length > 0) {
        ...
        if (matchingItem) {
          if (index < matchingItem.urls.length) {
            ...
            enviando = false  
            throw `*[❗] Tautan untuk angka tersebut tidak ditemukan, masukkan angka antara 1 dan ${matchingItem.urls.length}*`;
          }
        } else {
          enviando = false  
  ...
  try {
   ...
   if (fileSizeInMB > 100) {
    ...
    await conn.sendMessage(m.chat, {text: `*[ ✔ ] Video berhasil diunduh dan dikirim.*\n\n*—◉ Dikirim dalam format dokumen karena berat videonya ${roundedFileSizeInMB} MB dan melebihi batas yang ditetapkan oleh WhatsApp.*\n*◉ Titulo:* ${ttl_1}`, edit: key}, {quoted: m});
    enviando = false
   } else {
  ...
  try {
    ...
    try {
      ...
      await conn.sendMessage(m.chat, {video: {url: mediaa.result}, fileName: `error.mp4`, caption: `_Ciyo - Bot_`, thumbnail: mediaa.thumb, mimetype: 'video/mp4'}, {quoted: m});
      await conn.sendMessage(m.chat, {text: '*[ ✔ ] Video berhasil diunduh.*', edit: key}, {quoted: m});
      enviando = false
    } catch {
      try {
        ...
        throw '*[❗] Error, video tidak dapat diunduh.*';
      }
    }
  }
}};
